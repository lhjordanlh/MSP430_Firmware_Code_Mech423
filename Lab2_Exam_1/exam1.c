 ...
 * 3. Configure TA1 to produce a 500 Hz square wave at 80% duty cycle. Output on P1.6.
...
int main(void) {
	...
	BCSCTL1 |= RSEL3; // first step to set DCO to run 500 kHz
	BCSCTL1 &= ~RSEL1; // first step to set DCO to run 500 kHz
	BCSCTL1 &= ~RSEL0; // first step to set DCO to run 500 kHz

	DCOCTL &= ~(DCO1); // another line to help set DCO to run at 500 kHz
